...
function lose_dice(state::LDState, loser::Int)
    ...
    println("Press enter to continue")
    readline()
    # Reinitalize the state, the player who lost a dice starts
    ...
    while true
        ...
        if action == 1
            ...
            if challenge(state) # Challenge successful, original bidder loses a die
                state = lose_dice(state, state.prevTurn) 
            else # Challenge unsuccessful, challenger loses a die
                state = lose_dice(state, state.turn) 
            end
        end
...
# Lets initalize a game, num_player must align with size of num_dice and players
num_player = 3
num_dice = [4, 4, 4]
players = [ManualPlayer(1), POMDPPlayer(2, []), MDPPlayer(3)]
